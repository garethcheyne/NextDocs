@echo off
REM Script to run the app in development mode using the same database
...
REM Stop production app first (keep database/redis running)
echo ⏸️ Stopping production app (keeping database/redis)...
docker-compose -f docker-compose.prod.yml stop app

REM Make sure database and Redis are still running
echo 🗄️ Ensuring database and Redis are running...
docker-compose -f docker-compose.prod.yml up -d postgres redis

REM Build and start development container on same port
